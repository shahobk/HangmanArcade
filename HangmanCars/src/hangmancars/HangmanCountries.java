...
public class HangmanCountries extends Application {
    ...
    private static final float bonusMod = 0.4f; //amount by which the bonus will increase
    
    private SimpleStringProperty wordToGuess = new SimpleStringProperty(); //word that will need to be guessed
    
    private SimpleIntegerProperty lettersToGuess = new SimpleIntegerProperty(); //remaining letters
    ...
    private WordReaderCountries wordReader = new WordReaderCountries(); //reads from the textfile dedicated to it's specific game
    ...
    public Parent createContent(){ //superclass that will create the GUI
        ...
        playable.bind(hangman.lives.greaterThan(0).and(lettersToGuess.greaterThan(0)));//binds requirements (# of lives must be > 0, and letters to guess must be > 0) to the "playable" boolean value
        ...
        Button btnAgain = new Button("NEW GAME"); //creates a new button (new game)
        btnAgain.disableProperty().bind(playable); //will disable button until game is won or lost
        btnAgain.setOnAction(event -> startGame()); //when button is pressed, a new game is started
        
        Button btnExit = new Button("EXIT"); // creates a new button (exit)
        btnExit.setOnAction(event -> exitGame()); // when the button is pressed, the program will be closed.
        ...
        */
        HBox rowHangman = new HBox(10, btnAgain, btnExit, textScore, hangman); //"10" creates gaps between the elements
    ...
    private void stopGame(){
        ...
        wordToGuess.set(wordReader.getRandomWord().toUpperCase());//the word is retrieved from the text file and put into uppercase
        lettersToGuess.set(wordToGuess.length().get()); //# of letters left to guess is set to the length of the word since none have been guessed yet        
        ...
        public HangmanImage(){
         ...
         public Letter(char letter){
            ...
            bg.setFill(letter == ' ' ? Color.GREEN : Color.WHITE);
     ...
     public void start(Stage primaryStage) throws Exception {
            ...
            if (playable.get()){ //if the game is playable
                ...
                for (Node n : letters){
                    ...
                    if (letter.isEqualTo(pressed)){
                        ...
                        lettersToGuess.set(lettersToGuess.get() - 1); //one less letter to guess
                    ...
                    scoreMod += bonusMod; //bonus is added to modifier
                }
            }
});
        ...
        startGame();  
               ...
               System.exit(0); // When the exit button is selected, the program will close. 
          ...
          public static void main(String[] args) {
              ...
              launch(); // Sends the player from the menu, to the selected game they desire.
          }  
